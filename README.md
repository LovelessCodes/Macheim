...
- **Publish modpacks to Thunderstore** — sign in once with a service-account token (kept in the macOS Keychain, never shown again), then publish from the export sheet: pick a team, set the NSFW flag, watch upload progress, and open the published page when it is live. Versions are never auto-bumped; a duplicate version comes back as a clear API error
