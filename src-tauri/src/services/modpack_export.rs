...
/// Thunderstore requires the package icon to be exactly this size.
...
/// Thunderstore rejects anything but a 256x256 PNG, so icons are cropped to a
/// square and scaled to size. Already-correct icons pass through untouched.
fn normalise_icon(bytes: &[u8]) -> AppResult<Vec<u8>> {
    if png_dimensions(bytes) == Some((ICON_SIZE, ICON_SIZE)) {
        return Ok(bytes.to_vec());
    }

    let image = image::load_from_memory_with_format(bytes, image::ImageFormat::Png)
        .map_err(|e| AppError::Profile(format!("Could not read the icon: {}", e)))?;
    ...
    let resized = square.resize_exact(ICON_SIZE, ICON_SIZE, image::imageops::FilterType::Lanczos3);

    let mut out = Cursor::new(Vec::new());
    resized
        .write_to(&mut out, image::ImageFormat::Png)
        .map_err(|e| AppError::Profile(format!("Could not write the icon: {}", e)))?;
    Ok(out.into_inner())
}
...
pub fn build_modpack(
    ...
    let (icon_bytes, icon_warning) = match icon {
        Some(bytes) => {
            let needs_resize = png_dimensions(bytes) != Some((ICON_SIZE, ICON_SIZE));
            (
                normalise_icon(bytes)?,
                needs_resize.then(|| {
                    "The icon was cropped and resized to 256×256 for Thunderstore".to_string()
                }),
            )
        }
        None => (normalise_icon(DEFAULT_ICON)?, None),
    };
...
mod tests {
    ...
    #[test]
    fn zip_carries_a_readme_and_a_256_default_icon() {
        let (bytes, result) = build_modpack(&profile(), &metadata(), None, None).unwrap();
        ...
        assert_eq!(
            png_dimensions(&read_entry(&bytes, "icon.png")),
            Some((256, 256))
        );
    ...
    #[test]
    fn a_large_icon_is_cropped_and_resized_with_a_note() {
    ...
    #[test]
    fn a_non_square_icon_is_cropped_to_a_square() {
        let icon = png(512, 256);
